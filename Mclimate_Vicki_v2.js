...
function decodeUplink(input) {
  ...
  function handleKeepalive(bytes, data){
      ...
      // === Flattened assignments (primitive values) ===
      data.temperature_set = Number(bytes[1]);
      data.temperature = Number(sensorTemp.toFixed(2));
      data.relHumidity = Number(((bytes[3] * 100) / 256).toFixed(2));
      data.motorRange = motorRange;
      data.motorPosition = motorPosition;
      data.battery_vdd = Number(batteryVoltageCalculated.toFixed(2)) * 1000;
      data.windowOpen = toBool(openWindow);
      ...
      data.freezeProtection = toBool(antiFreezeProtection);
      data.valveOpening = motorRange != 0 ? Math.round((1-(motorPosition/motorRange))*100) : 0;
